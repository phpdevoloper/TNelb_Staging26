...
$("#competency_form_a").on("submit", function (e) {
    ...
    let qcStaffRowCount = $("#staffqc-records tr").length;
    if (qcStaffRowCount === 0) {

        isValid = false;

        // Make entire table border red
    ...
    if (mandatoryStaffRowCount < 2) {
        ...
        $("#staff-container").after(
            '<span class="staff-validation-error error text-danger d-block mt-2">' +
    ...
    $mandatoryStaffRows.slice(0, 2).each(function (index) {

        let $row = $(this);

        // Category
        let category = $row
            .find('input[name="staff_category[]"], select[name="staff_category[]"]')
            .val();

        // Certificate Number
        let ccNumber = $row.find(".cc_number").val() || "";
        ccNumber = ccNumber.trim();

        // First Issue
        let firstIssue = $row.find(".cc_firstissue").val() || "";
        firstIssue = firstIssue.trim();

        // Validity From
        let validityFrom = $row.find(".cc_validity_from").val() || "";
        validityFrom = validityFrom.trim();

        // Validity To
        let validityTo = $row.find(".cc_validity_to").val() || "";
        validityTo = validityTo.trim();
        ...
        if (category !== "B") {
            ...
            $row.find(
                'input[name="staff_category[]"], select[name="staff_category[]"]'
            ).after(
                '<span class="staff-validation-error error text-danger d-block">' +
                'Category B is mandatory for Staff ' + (index + 1) +
                '.</span>'
            );
        }
        ...
        if (firstIssue === "") {
            ...
            $row.find(".cc_firstissue").after(
                '<span class="staff-validation-error error text-danger d-block">' +
                'Certificate First Issue is required.' +
                '</span>'
            );
        }
        ...
        if (validityFrom === "") {
            ...
            $row.find(".cc_validity_from").after(
                '<span class="staff-validation-error error text-danger d-block">' +
                'Certificate Validity From is required.' +
                '</span>'
            );
        }
        ...
        if (validityTo === "") {
            ...
            $row.find(".cc_validity_to").after(
                '<span class="staff-validation-error error text-danger d-block">' +
                'Certificate Validity To is required.' +
                '</span>'
            );
        }
    ...
    // ======================================================
    // B STAFF VALIDATION FAILED
    // ======================================================

    if (!staffValidationPassed) {
        ...
        Swal.fire({
            ...
            text: "Please complete all mandatory details for the first 2 B staff members.",
            confirmButtonText: "OK",
            width: 500
        });
...
$("#competency_form_a_return").on("submit", function (e) {
    ...
    if (isValiddraft) {
        if (actionType === "draft") {
            submitFormAFinal_return(formData, actionType);
            return;
        }
    }
    // alert(actionType);
...
function checkvaliditydatesformA_return(formData, actionType = "") {
    let firstCertNo = $("input[name='cc_number[]']").eq(0).val()?.trim();
    let firstCertValidity = $("input[name='cc_validity[]']")
        .eq(0)
        .val()
...
function submitFormAFinal_return(formData, actionType) {
    ...
    let postUrl =
        applType === "R"
            ? BASE_URL + "/forma/storerenewalreturn"
            : BASE_URL + "/forma/storereturn";
    //  let amount = formData.get("fees") || 0;
...
flatpickr(
    'input[name="previous_validity_first_issue"],' +
    'input[name="previous_validity_from"],' +
    'input[name="previous_validity_to"],' +
    'input[name="staff_cc_first_issue[]"],' +
    'input[name="staff_cc_validity_from[]"],' +
    'input[name="staff_cc_validity_to[]"],' +
    'input[name="cc_validity[]"],' +

    'input[name="cc_firstissue[]"],' +
    'input[name="cc_validity_from[]"],' +
    'input[name="cc_validity_to[]"]',
    {
        dateFormat: "d-m-Y"
    }
);
