/**
 * Competency forms S / W / WH — Pay, validation, preview, draft save, payment.
 * URLs come from window.COMPETENCY_FORM_CONFIG (injected in footer.blade.php via Laravel route()).
 */
(function (window, $) {
    'use strict';

    if (!$ || !$.fn) {
        console.error('competency_form_ws.js requires jQuery');
        return;
    }
    ...
    async function showDeclarationPopup(licence_code, directProceed = false) {
        ...
        try {
            ...
            let total_fees,actual_fees, renewl_fees, lateFee, lateMonths, form_cost, form_name, licence, renewalAmoutStartson, latefee_amount, latefee_starts, form_instruct, fees_date, certificate_name;
            ...
            const noPaymentApplType = (typeof window.isNoPaymentApplType === 'function')
    ...
    function showPaymentSuccessPopup(loginId, transactionId, transactionDate, applicantName, amount, form_type, licence_name, isFormP, options) {
        ...
        const isFeeExemptSubmit = (typeof window.isFeeExemptCompetencySuccess === 'function')
            ? window.isFeeExemptCompetencySuccess(loginId, form_type, options.feeExempt === true)
    ...
    function downloadPDF(language) {
        ...
        if (window.paymentIsFormP) {
            ...
            url = (language === 'tamil')
                ? `${(cfg.baseUrl || (typeof BASE_URL !== "undefined" ? BASE_URL : "")).replace(/\/$/, "")}/generatePDFFormPTA/${window.paymentAppId}`
                : `${(cfg.baseUrl || (typeof BASE_URL !== "undefined" ? BASE_URL : "")).replace(/\/$/, "")}/generate-pdf-p/${window.paymentAppId}`;
        } else {
            url = (language === 'tamil')
                ? `${(cfg.baseUrl || (typeof BASE_URL !== "undefined" ? BASE_URL : "")).replace(/\/$/, "")}/generateTamilPDF/${window.paymentAppId}`
                : `${(cfg.baseUrl || (typeof BASE_URL !== "undefined" ? BASE_URL : "")).replace(/\/$/, "")}/generate-pdf/${window.paymentAppId}`;
        }
    ...
    function paymentreceipt() {
        ...
        window.open(`${(cfg.baseUrl || (typeof BASE_URL !== "undefined" ? BASE_URL : "")).replace(/\/$/, "")}/payment-receipt/${window.paymentAppId}`, "_blank");
    }

    window.getPaymentsService = getPaymentsService;
    window.showDeclarationPopup = showDeclarationPopup;
    ...
    $(document).ready(function () {
        ...
        (function showPayUSuccessPopupFromQuery() {
            try {
                ...
                if (params.get('payu_success') !== '1') {
                    return;
                }
                if (typeof showPaymentSuccessPopup !== 'function' && typeof window.showPaymentSuccessPopup !== 'function') {
                    return;
                }
                if (!$('#paymentSuccessModal').length) {
                    return;
                }
                ...
                const applicantName = params.get('applicant_name') || 'N/A';
                const licenceName = params.get('licence_name') || 'N/A';
                const formType = params.get('form_type') || 'New Application';
                ...
                // Clean query string so refresh does not reopen the modal forever
                if (window.history && window.history.replaceState) {
                    const cleanUrl = window.location.pathname + (window.location.hash || '');
                    window.history.replaceState({}, document.title, cleanUrl);
                }
            } catch (e) {
                console.error('PayU success popup restore failed:', e);
            }
        })();
        ...
        function isFeeExemptCompetencySuccess(applicationId, formType, feeExemptHint) {
            ...
            if (typeof isNoPaymentApplType === 'function' && window.isNoPaymentApplType()) {
                return true;
            }
