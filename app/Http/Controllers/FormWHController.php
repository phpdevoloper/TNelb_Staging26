...
/**
 * Form WH applicant facade for New, Renewal, Digitisation, and Alteration.
...
class FormWHController extends BaseController
{
    ...
    public function digitize(Request $request)
    {
        ...
        $request->merge(['form' => $request->input('form', FormWHSchema::FORM_NAME)]);
        $request->merge(['form' => FormWHSchema::canonical($request->input('form'))]);
        $request->validate([
            'form' => 'required|in:WH',
        ]);
    ...
    public function getContractorDetails($loginId, $tempAppId = null, $applicationId = null)
    {
        ...
        if (! empty($applicationId)) {
            ...
            $query->where('form_name', FormWHSchema::FORM_NAME);
        }
    ...
    {
        $request->merge([
            'form_name' => FormWHSchema::FORM_NAME,
            'cert_name' => FormWHSchema::LICENSE_NAME,
        ]);
        $request->validate([
            ...
            'form_name' => 'required|in:WH',
            'cert_name' => 'required|in:H',
        ], [
        ...
        $record = DB::transaction(function () use (
            ...
            $row = Tnelb_CC_Digitization::create([
                ...
                'cc_type' => FormWHSchema::LICENSE_NAME,
            ]);

            $temp_app_id = 'TEMP'.date('Ymd').str_pad($row->id, 4, '0', STR_PAD_LEFT);
            $digiDir = CompetencyDocumentSupport::digitizationUploadDirectory(FormWHSchema::FORM_NAME);
    ...
    {
        $request = request();
        $request->attributes->set(FormWHSchema::VIA_CONTROLLER_ATTR, true);
    ...
    public function alterIndex(Request $request)
    {
        ...
        $request->merge(['form' => FormWHSchema::FORM_NAME]);
        ...
        if ($parentId === '') {
            return view('user_login.alteration.form_wh_launcher', [
                ...
                'alterCertificatesUrl' => route('form_wh_alt.certificates'),
            ]);
        }

        if (! $this->alterationService->isLauncherVerifiedFor(FormWHSchema::FORM_NAME, $parentId)
        ...
        if (! $verify['ok'] || ! FormWHSchema::isFormWH($verify['application']->form_name ?? '')) {
            return redirect()
                ->route('form_wh_alt')
    ...
    public function listCertificates(Request $request)
    {
        ...
        $certificates = $this->alterationService->listIssuedCertificatesForLogin($loginId, FormWHSchema::FORM_NAME);

        return response()->json([
            'status' => 'success',
            'form' => FormWHSchema::FORM_NAME,
    ...
    public function verifyParent(Request $request)
    {
        ...
        $this->alterationService->markLauncherVerified(FormWHSchema::FORM_NAME, $applicationId, $certificateDetails);

        return response()->json([
            'status' => 'success',
            'message' => 'Certificate verified successfully.',
            'application_id' => $applicationId,
            'redirect_url' => route('form_wh_alt', [
    ...
    public function storeAlteration(Request $request)
    {
        ...
        try {
            ...
            if (! FormWHSchema::isFormWH($child->form_name ?? '')) {
                throw new \RuntimeException('Alteration is not a Form WH application.');
            }
    ...
    {
        $request->merge(FormWHSchema::identityPayload());
        $request->attributes->set(FormWHSchema::VIA_CONTROLLER_ATTR, true);
